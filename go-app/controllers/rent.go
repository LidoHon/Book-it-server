...
func DeleteRent() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// to do
		// update the book avaliabilty to true for that specific book after deleting the rent
...
func ReturnBook() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		var req requests.ReturnBookRequest
		if err := c.ShouldBindJSON(&req); err != nil {
			c.JSON(http.StatusBadRequest, gin.H{"message": "Invalid input", "details": err.Error()})
			return
		}

		// Step 1: Remove the book from the rented table
		deleteMutation := struct {
			DeleteRentedBooks struct {
				Returning []struct {
					Id int `json:"id"`
				} `json:"returning"`
			} `graphql:"delete_rentedBooks(where: {bookId: {_eq: $bookId}})"`
		}{}
		mutationVars := map[string]interface{}{
			"bookId": graphql.Int(req.Input.BookId),
		}
		err := client.Mutate(ctx, &deleteMutation, mutationVars)
		if err != nil {
			log.Println("Failed to return the book:", err)
			c.JSON(http.StatusInternalServerError, gin.H{"message": "Failed to delete rent for the book", "details": err.Error()})
			return
		}

		// Step 2: Update the book availability to true
		updateBookMutation := struct {
			UpdateBook struct {
				Id        int  `json:"id"`
				Available bool `json:"available"`
			} `graphql:"update_books_by_pk(pk_columns: {id: $bookId}, _set: {available: $available})"`
		}{}
		updateBookMutationVars := map[string]interface{}{
			"bookId":    graphql.Int(req.Input.BookId),
			"available": graphql.Boolean(true),
		}

		err = client.Mutate(ctx, &updateBookMutation, updateBookMutationVars)
		if err != nil {
			log.Println("Failed to update book availability:", err)
			c.JSON(http.StatusInternalServerError, gin.H{"message": "Failed to update book availability", "details": err.Error()})
			return
		...
		// Step 3: Fetch the book title and check if it's in any user's wishlist
		var wishlistQuery struct {
			UsersWithBookWishlist []struct {
				Id int `json:"id"`
			} `graphql:"users(where: {wishlists: {bookId: {_eq: $bookId}}})"`
			Book struct {
		...
		// Step 4: Send notifications to users who have wishlisted this book
		for _, user := range wishlistQuery.UsersWithBookWishlist {
			notificationMutation := struct {
				InsertNotification struct {
					Id int `json:"id"`
				} `graphql:"insert_notification_one(object: {message: $message, userId: $userId})"`
			}{}
		...
		res := models.BookReturnOUtput{
			Message: "Book returned successfully, and notifications sent where applicable",
		}
